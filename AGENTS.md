...
- **Never use a platform `#if` to silence an API-availability error inside an adaptive view.** `AppShellView` picks its shell by `horizontalSizeClass`, so `SidebarShell` runs on iPad *and* Mac. Its sidebar list was wrapped in `#if !os(iOS)` to get past `List(_:id:selection:)` being unavailable on iOS — which on iPad compiled the entire sidebar away and shipped a blank third of the screen. A platform `#if` claims "this platform doesn't need it"; an unavailable API means "this platform spells it differently". Resolve the difference (iOS takes an optional selection binding) rather than deleting the view. iPad is a shipping target — check `TARGETED_DEVICE_FAMILY` — and nothing in the build or test loop exercises it, so a regression here is invisible until someone looks at an iPad. Capture one: `Tools/capture-screenshots.sh <out> <ipad-udid>`.

- **Hiding a category of content takes two edits, not one.** The Music tab, the Home rails and the Settings library list all derive from `AppState.libraries`, so filtering that array hides a whole media type at once (`FeatureFlags.musicEnabled` does exactly this). Search does **not** go through it — `filteredSearch` queries the server directly — so anything filtered out of the library list still comes back in search results until `SearchView` filters it too. Assume the same of any future surface that talks to the provider instead of reading `libraries`. Note also that hiding a tab does not make it unreachable: `AppTab.availableTabs` only decides what the tab bar and sidebar *list*, while the shell renders whatever `appState.selectedTab` holds without consulting it. Anything that sets the tab directly — a deep link, restored state, `ScreenshotDriver` — will still render a hidden section's view.

- **`Cove/README.md` states intent, not fact.** It is the design pitch — it lists tvOS and watchOS in its platform table, promises gapless playback and a server-agnostic backend for Plex and Navidrome, and calls the whole thing early development. Some of that is built and some is aspiration, and the file does not distinguish. Never source a feature claim from it: for anything user-facing — App Store copy, release notes, a capability list — verify against the code that would implement it (the Settings screens, `DownloadCoordinator`, `VideoPlaybackManager`, `CoveWidget`) and against the target's actual `SUPPORTED_PLATFORMS` and `TARGETED_DEVICE_FAMILY` in the pbxproj. Claiming a feature the binary does not have is an App Store rejection, not a typo. The same goes for privacy claims, in the other direction: before writing that the app talks to nothing but your server, enumerate what it can actually reach — `grep -rhoE 'https://[a-zA-Z0-9._-]+' Cove Packages/CoveKit/Sources CoveWidget | sort -u` — and check each hit, since a URL the app only *builds* for the user to tap (IMDb, TMDB, TheTVDB) is not a request it makes, while one behind an opt-in user-supplied API key (OpenSubtitles) still has to be disclosed.
