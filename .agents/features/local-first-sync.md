...
**Status:** phases 0–5 implemented; see §11 for what each proved and what stays open. Every load-bearing claim below was checked against the
...
`UserDataStore` keeps its API. **As built:** with an outbox configured it enqueues and
never rolls back; the in-memory overrides stay for instant UI; without a database (previews)
it falls back to the old call-the-server path. The protection rule in §7.5 is enforced in
the repository, so it holds for sweeps, deltas and detail refreshes alike.
...
- Phase 4 live: an outbox row for a favourite, written directly into the simulator's database, flipped the demo server's `IsFavorite` on the next launch and was cleared; the local flag survived the sweep that followed. `POST /UserPlayedItems?datePlayed=` and `POST /UserItems/{id}/UserData` both return 200 on 12.0 (from Phase 0). All writes to the shared demo account were reverted.
